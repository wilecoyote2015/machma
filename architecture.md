...
**Keyboard shortcuts:** `TimelineView` stores the React Flow instance via `onInit` and registers a window-level `keydown` listener for Ctrl+F → `fitView`. The listener uses a ref for the current `fitViewOptions` (padding + filtered task node list) so it always fits to the currently visible nodes. Because the listener is on `window`, it fires regardless of which panel (canvas, filter, detail) the mouse hovers over. The effect auto-cleans up when `TimelineView` unmounts, so it only applies while the timeline view is active.
